...
The Vignette is used to guide users to perform differential protein expression 
analysis:
...
`DEqMS` works on top of Limma, a widely-used R package for microarray data 
analysis (Smyth G. et al 2004), and improve it with mass spectrometry specific 
data properties, variance dependence on peptide spectra count for statistical 
testing of differential protein expression. 
...
## 3 Differential protein expression analysis
### 3.1 Example Dataset: A431 cells treated with three different miRNA mimics
we analyzed a protemoics dataset (TMT10plex labelling) in which A431 cells 
...
statistical power by borrowing informations across genes to shrink the variance. 
...
all rows instead of the top 10.
...
plot(log2(psm.count.table[rownames(dat.gene.nm),2]),
     ...
     main="variation between two control replicates")

boxplot(log2(var)~PSMcount,dat.temp.filter,xlab="PSMcount",
        ylab="log2(Variance)",main="variance dependence on PSM count")
...
variance estimate for genes identified with different number of PSMs. It works 
...
y = fit3$s2.post
op <- par(mfrow=c(1,2), mar=c(4,4,4,1), oma=c(0.5,0.5,0.5,0))
plot(log2(x),log2(y),pch=20,cex=0.5 , xlab = "log2(PSMcount)",
     ylab="log2(Variance)",
...
### 5.1 PSM/Peptide profile plot
`make.profile.plot` function will plot log2 intensity of each PSM/peptide of 
